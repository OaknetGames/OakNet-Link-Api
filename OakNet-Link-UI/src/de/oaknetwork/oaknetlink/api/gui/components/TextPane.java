...
/**
 * A textPane holds a large amount of text and provides automatic line wrap.
 * 
 * @author Fabian Fila
 */
public class TextPane extends ColorComponent {
	...
	/**
	 * Appends some text in a new line
	 * 
	 * @param text
	 */
	...
	/**
	 * Replaces the current shown text with a new text
	 * 
	 * @param text
	 */
	...
	public void renderComponent(Vector2i position) {
		...
		if (index < 1)
			index = 1;
		while (index < linePointer
				&& textPos.Y - position.Y < size().Y - 2 * outlineSize - MinecraftHooks.mcInterface.stringHeight()) {
